...
#define STRING_CONFIG_H_AUTHOR \
    "(Dust, valerionew, Ender-3)"  // Who made the changes.
 ...
 * Thermistor 67 : 500kΩ  SliceEngineering 450°C Thermistor 68 : PT100 amplifier
 * board from Dyze Design 70 : 100kΩ  bq Hephestos 2 75 : 100kΩ  Generic Silicon
 * Heat Pad with NTC100K MGB18-104F39050L32 2000 : 100kΩ  Ultimachine Rambo TDK
 * NTCG104LH104KT1 NTC100K motherboard Thermistor
 *
...
#define HOTEND_OVERSHOOT \
    15                    // (°C) Forbid temperatures over MAXTEMP - OVERSHOOT
...
#define MPC_STEADYSTATE \
    0.5f  // (K/s) Temperature change rate for steady state logic to be
          // enforced.

#define MPC_TUNING_POS           \
    {                            \
        X_CENTER, Y_CENTER, 1.0f \
    }  // (mm) M306 Autotuning position, ideally bed center at first layer
       // height.
...
#define PID_FUNCTIONAL_RANGE \
    10  // If the temperature difference between the target temperature and the
        // actual temperature is more than PID_FUNCTIONAL_RANGE then the PID
        // will be shut off and the heater will be set to min/max.
...
// uncomment to add G33 Delta Auto-Calibration (Enable EEPROM_SETTINGS to store
// results)
...
#define DEFAULT_ANGULAR_TRAVEL_ACCELERATION \
    3000  // I, J, K acceleration for rotational-only travel (non printing)
          // moves
#endif
...
#define JD_HANDLE_SMALL_SEGMENTS    // Use curvature estimation instead of just
                                    // the junction angle for small segments (<
// 1mm) with large junction angles (> 135°).
#endif
...
#define NUM_RUNOUT_SENSORS \
    1  // Number of sensors, up to one per extruder. Define a FIL_RUNOUT#_PIN
       // for each.
...
//#define BED_TRAMMING_INCLUDE_CENTER       // Move to the center after the last
// corner #define BED_TRAMMING_USE_PROBE
...
#define NOZZLE_PARK_MOVE \
    0  // Park motion: 0 = XY Move, 1 = X Only, 2 = Y Only, 3 = X before Y, 4 =
       // Y before X
...
#define NEOPIXEL_PIXELS \
    30  // Number of LEDs in the strip. (Longest strip when NEOPIXEL2_SEPARATE
        // is disabled.)
